...
两档模式, 由 workflow 按 cron 决定:
  ...
  MODE=slow  每小时:     巨潮港股公告 + 东财个股新闻(逐家)。全中文来源。
...
# ── slow: 巨潮港股公告 + 东财个股新闻(全中文)────────────────────────────────
def collect_slow():
    """慢线已停用: 按用户要求, 只保留快线的 财联社/新浪/东财快讯 + 东财公告大全。
    港股公告与个股新闻不再抓取。保留此函数只是为了 workflow 里 slow 触发时不报错。"""
    return [], []
...
def smoketest():
    ...
    fake_intl = [mk_item([c for c in COMPANIES if c["group"] == "国外"][0],
                         ...
                         now - dt.timedelta(minutes=8), "https://y", "东方财富", "新闻")]
    g = globals()
