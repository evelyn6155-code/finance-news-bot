...
# -*- coding: utf-8 -*-
"""
cn_ai_monitor.py —— 国内 AI 公司「重大公告 + 多源快讯 + 行情」采集器 (宁精勿滥版)
================================================================================
口径: 只报重大公告 + 财联社「重点」等快讯命中, 股价降为辅助一行, 每条可溯源。
职责: 只做“抓取 + 去重 + 出结构化清单”; 成稿(DeepSeek)+发信(QQ)在 write_and_send.py。

数据源(均 akshare 封装, 已核实):
  公告  —— 巨潮资讯网  ak.stock_zh_a_disclosure_report_cninfo (A股+港股, 仅保留重大类别)
  快讯  —— 四源合并去重:
           财联社·重点  ak.stock_info_global_cls(symbol="重点")
           新浪财经      ak.stock_info_global_sina
           同花顺        ak.stock_info_global_ths     (带原文链接)
           东方财富      ak.stock_info_global_em       (带原文链接)
  行情  —— 东方财富     ak.stock_zh_a_spot_em / ak.stock_hk_spot_em   (辅助)
  (金十数据无稳定接口, 本版暂不接; 华尔街见闻反爬硬, 不接。)

运行:
  MODE=head python cn_ai_monitor.py     # 头部, 回溯 24h
  MODE=full python cn_ai_monitor.py     # 全量, 回溯 72h
  python cn_ai_monitor.py --selftest    # 离线自检(不联网)
输出: report.md (给 DeepSeek) + report.json
"""
...
def _parse(ts):
    ...
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d"):
        try:
            ...
            continue
    m = re.match(r"^(\d{1,2}):(\d{2})(:\d{2})?$", s)   # 只有 HH:MM(:SS) → 按今天
    if m:
...
# ── 快讯(四源合并)────────────────────────────────────────────────────────────
def fetch_flash_all():
    """一次性拉四个快讯源, 归一化为 [{title,text,time,src,url}]。"""
    rows = []
    ak = _ak()
    def grab(fn, kw, mk):
        try:
...
def build(mode, flash=None, quotes=None, _mock=None):
    start, end = window(mode)
    scope = [c for c in COMPANIES if mode == "full" or c["tier"] == "head"]
    flash = flash if flash is not None else (fetch_flash_all() if _mock is None else [])
    quotes = quotes if quotes is not None else (fetch_spot() if _mock is None else {})
    records, errors = [], [f["_err"] for f in flash if "_err" in f]
    for c in scope:
        items = []
        if _mock is not None:
            items = list(_mock.get(c["name"], []))
        else:
...
def render_md(report):
    ...
    L = [f"# 国内AI公司要闻清单(宁精勿滥) | {end:%Y-%m-%d %H:%M} "
         f"(回溯至 {start:%m-%d %H:%M}, MODE={report['mode']})", ""]
    empty = []
...
def selftest():
    ...
    mock = {
        "寒武纪": [
            {"title": "寒武纪：关于股价异动的风险提示公告", "time": now - dt.timedelta(hours=3),
             "url": "http://www.cninfo.com.cn/x", "src": "巨潮资讯网·公告"},
            {"title": "算力芯片走强，寒武纪盘中涨超12%创新高", "time": now - dt.timedelta(hours=2),
             "url": "https://news.10jqka.com.cn/y", "src": "同花顺快讯"},
            {"title": "算力芯片走强，寒武纪盘中涨超12%创新高", "time": now - dt.timedelta(hours=2),
             "url": "https://news.10jqka.com.cn/y", "src": "东方财富快讯"},  # 跨源重复, 应去重
        ],
        "智谱AI": [
            {"title": "智谱：拟配售新H股募资约314亿港元", "time": now - dt.timedelta(hours=20),
             "url": "https://www.cls.cn/telegraph", "src": "财联社·重点"},
        ],
        "科大讯飞": [],
    }
    quotes = {"688256": {"price": 1372.0, "chg": 12.3, "ytd": 210.0},
              "02513": {"price": 1834.0, "chg": 0.49, "ytd": None}}
    global COMPANIES
    COMPANIES = [c for c in COMPANIES if c["name"] in mock]
    rep = build("head", flash=[], quotes=quotes, _mock=mock)
    print(render_md(rep))
    cam = next(r for r in rep["records"] if r["company"] == "寒武纪")
    assert len(cam["items"]) == 2, f"跨源去重失败: {len(cam['items'])}"
    assert any(r["company"] == "科大讯飞" for r in rep["records"])
    print("\n[selftest] OK — 四源装配 / 跨源去重 / 空动态归并 / 渲染 均通过")
